const ctxProgress = document
  .getElementById("shimmerProgressBar")
  .getContext("2d");

// Ensure canvas has the correct dimensions
const progressCanvas = document.getElementById("shimmerProgressBar");
progressCanvas.width = 200;
progressCanvas.height = 200;
...
function animateProgress() {
  ...
  const startPercentage =
    currentStage === 0 ? 0 : targetPercentages[currentStage - 1];
  ...
  function animate(currentTime) {
    ...
    ctxProgress.clearRect(
      0,
      0,
      ctxProgress.canvas.width,
      ctxProgress.canvas.height
    );
    ...
    if (progress < 1) {
      ...
      currentStage++;
      setTimeout(animateProgress, 1000); // Wait 0.5 seconds before starting next stage
    }
  }
