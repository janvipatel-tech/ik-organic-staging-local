...
if (copyResultsLink) {
    copyResultsLink.addEventListener("click", function () {
        const url = window.location.origin + "/big-tech?utm_source=result-referral-button";
        navigator.clipboard
            .writeText(url)
            .then(function () {
...
function getResumeAnalysisUrl(domain, experience, country) {
//     const RESUME_ANALYSIS_API_URL = "https://ueeidhfhl3.execute-api.us-west-1.amazonaws.com/prod/ai-roadmap";
    ...
    return RESUME_ANALYSIS_API_URL;
}

const RESULTS_POLL_INTERVAL_MS = 2000;
const RESULTS_MAX_ATTEMPTS = 60;
const RESULTS_FAILURE_REDIRECT = "/big-tech-road-map/";
const SUCCESS_STATUSES = new Set(["success", "completed", "done", "ready", "ok"]);
const FAILURE_STATUSES = new Set(["failed", "error", "timeout", "cancelled"]);
const PENDING_STATUSES = new Set(["pending", "processing", "in_progress", "queued", "analysing", "created"]);

function delay(ms) {
    return new Promise((resolve) => setTimeout(resolve, ms));
}

function createResultsUrl(baseUrl, params) {
...
async function pollForAnalysisResults(baseUrl, params, headers = {}) {
    const pollUrl = createResultsUrl(baseUrl, params);
    let attempt = 0;
    let lastError = null;

    while (attempt < RESULTS_MAX_ATTEMPTS) {
        attempt += 1;
        try {
            ...
            if (rawBody) {
                try {
                    ...
                    console.warn("Failed to parse analysis response JSON:", parseError);
                }
            }

            const statusFromBody =
                normalizeStatus(parsedBody?.status) ||
                normalizeStatus(parsedBody?.data?.status);

            if (response.ok) {
                if (SUCCESS_STATUSES.has(statusFromBody) || (!statusFromBody && parsedBody?.data)) {
                    return parsedBody;
                }

                if (FAILURE_STATUSES.has(statusFromBody)) {
                    throw new Error(parsedBody?.message || "Analysis request failed.");
                }

                if (
                    response.status === 202 ||
                    PENDING_STATUSES.has(statusFromBody) ||
                    (!parsedBody && response.status < 400)
                ) {
                    await delay(RESULTS_POLL_INTERVAL_MS);
                    continue;
                }
                ...
                if (response.status === 202 || PENDING_STATUSES.has(statusFromBody)) {
                    await delay(RESULTS_POLL_INTERVAL_MS);
                    continue;
                }

                throw new Error(parsedBody?.message || `Request failed with status ${response.status}`);
            }
        } catch (error) {
            lastError = error;
            console.warn(`Analysis polling attempt ${attempt} failed:`, error);
            if (attempt >= RESULTS_MAX_ATTEMPTS) {
                throw lastError || new Error("Unable to fetch analysis results.");
            }
            await delay(RESULTS_POLL_INTERVAL_MS);
        }
    }

    throw lastError || new Error("Timed out while waiting for analysis results.");
}

function handleAnalysisFailure(message) {
    const errorText =
        message ||
        "We could not fetch your analysis right now. You will be redirected to the Big Tech page to try again.";
    alert(errorText);
    const isSameOriginReferrer =
        document.referrer && document.referrer.includes(window.location.origin)
            ? document.referrer
            : null;
    window.location.href = isSameOriginReferrer || RESULTS_FAILURE_REDIRECT;
}
...
async function getUserDetails() {
    try {
        ...
        if (!urlParams.domain || !urlParams.experience || !urlParams.country) {
            alert("Missing required parameters: domain, experience, or country");
            return;
        }

        const resultsURL = getResumeAnalysisUrl(urlParams.domain, urlParams.experience, urlParams.country);
        ...
        try {
            ...
            if (storedData) {
                ...
                console.log('Retrieved form data from localStorage:', storedFormData);
                localStorage.setItem("webinar_user_name", storedFormData.full_name || "");
                localStorage.setItem("webinar_user_email", storedFormData.email || "");
                localStorage.setItem("webinar_user_phone", storedFormData.phone || "");
                localStorage.setItem("webinar_user_domain", storedFormData.domain || "");
                localStorage.setItem("webinar_user_experience", storedFormData.experience || "");
            }
        } catch (error) {
            console.log('Error retrieving form data from localStorage:', error);
        }

        // Prepare query parameters for analysis request (only required fields)
        ...
        console.log('Polling results API with params:', analysisQueryParams);

        const timeLineData = await pollForAnalysisResults(resultsURL, analysisQueryParams, {
            'x-api-key': 'Q6XgZ2uukP7BLeDGIP1467PKHOjMsLRs2VL9E53q'
        });

        if (!timeLineData || !timeLineData.data) {
            throw new Error('Analysis response did not include expected data payload.');
        }
        ...
        // Use stored form data or default values
        const name = storedFormData?.full_name || "User";
        const email = storedFormData?.email || "user@example.com";
        const phone_number = storedFormData?.phone || "";
        ...
        document.querySelectorAll(".register-session-slot").forEach((button) => {
            ...
            button.addEventListener("click", () => {
                ...
                const userEmail = storedFormData?.email || prompt("Please enter your email to register for the webinar:");
                if (userEmail) {
                    oneClickRegister(userEmail, slotValue, button);
                }
            });
        });
        ...
        jQuery(document).ready(function ($) {
            ...
            if (viewAllCourse) {
                viewAllCourse.addEventListener("click", function (e) {
                    if (v_country === "India") {
                        e.preventDefault();
                        window.open("https://staging-4eb7-ininterviewkickstart.wpcomstaging.com/?utm_source=L10x&utm_campaign=L10x_Big_tech_roadmap", "_blank");
                    }
                });
            }

            if (v_country === "India") {
                course1_link = "https://staging-4eb7-ininterviewkickstart.wpcomstaging.com/applied-generative-ai-for-tech-professionals-course/";
                course2_link = "https://staging-4eb7-ininterviewkickstart.wpcomstaging.com/#interview_courses";
                course3_link = "https://staging-4eb7-ininterviewkickstart.wpcomstaging.com/course/flagship-machine-learning-course/";
            }
            $("#course-card-title-1").text(course1);
            $("#course-card-desc-1").text(course1_description);
            $("#course-card-link-1").attr("href", course1_link + "?utm_source=L10x&utm_campaign=L10x_Big_tech_roadmap");

            $("#course-card-title-2").text(course2);
            $("#course-card-desc-2").text(course2_description);
            $("#course-card-link-2").attr("href", course2_link + "?utm_source=L10x&utm_campaign=L10x_Big_tech_roadmap");

            $("#course-card-title-3").text(course3);
            $("#course-card-desc-3").text(course3_description);
            $("#course-card-link-3").attr("href", course3_link + "?utm_source=L10x&utm_campaign=L10x_Big_tech_roadmap");
        ...
        handleAnalysisFailure(error?.message);
    }
